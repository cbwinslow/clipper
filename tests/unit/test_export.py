...
@patch("vaclip.export.clip_exporter.subprocess.run")
def test_clip_exporter_export_with_subtitles(mock_subprocess_run, tmp_path):
    def mock_run(*args, **kwargs):
        output_path = Path(args[0][-1])
        output_path.parent.mkdir(parents=True, exist_ok=True)
        output_path.write_text("dummycontent")
        return subprocess.CompletedProcess(args[0], 0, stdout="", stderr="")
    ...
    caption_text = "This is a test subtitle"
    clips = exporter.export(media, [clip_segment], framing="wide", max_clips=1, caption=caption_text)
    mock_subprocess_run.assert_called_once()
    args, kwargs = mock_subprocess_run.call_args
    assert "ffmpeg" in args[0]
    assert "-i" in args[0]
    ...
    assert "-to" in args[0]
    assert "20.0" in args[0]
    vf_index = args[0].index("-vf")
    vf_value = args[0][vf_index + 1]
    assert "scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:-1:-1:color=black" in vf_value
    assert "drawtext" in vf_value
    escaped_caption = caption_text.replace("'", "'\\''")
    assert f"text='{escaped_caption}'" in vf_value
...
@patch("vaclip.export.clip_exporter.subprocess.run")
def test_clip_exporter_export_with_empty_subtitles(mock_subprocess_run, tmp_path):
    def mock_run(*args, **kwargs):
        output_path = Path(args[0][-1])
        output_path.parent.mkdir(parents=True, exist_ok=True)
        output_path.write_text("dummycontent")
    ...
    clips = exporter.export(media, [clip_segment], framing="wide", max_clips=1, caption="")
    mock_subprocess_run.assert_called_once()
    args, kwargs = mock_subprocess_run.call_args
    assert "ffmpeg" in args[0]
    assert "-i" in args[0]
